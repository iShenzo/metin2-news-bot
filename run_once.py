...
def run():
    ...
    for cat in CATEGORIES:
        ...
        if cat.get("single_thread"):
            ...
            # —— Alle anderen Kategorien: alle Threads durchgehen —— #
            ...
            for tid, title, url, list_time in threads:
                # In DB anlegen, falls neuer Thread
                c.execute(
                    "INSERT OR IGNORE INTO threads(category, thread_id, last_post_id) VALUES (?,?,0)",
                    (name, tid)
                )
                ...
                # nur posten, wenn neuer in der letzten Stunde
                if post_time and post_time > cutoff and post_id > last_seen:
                    print(f"[{name}] Neuer Post in Thread “{title}” (#{post_id}), sende an Discord…")
                    ...
                    print(f"[{name}] Kein neuer Post in “{title}” (letzter gesehen: #{last_seen}).")
